...
const V2Page = lazy(() => import("./V2Page"))
...
function img(k: string) { return urls[k] || "" }
...
function useFade() {
  ...
  return { ref, cls: `transition-all duration-700 ease-out ${v ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}` }
}
...
function Nav() {
  ...
  return (
    <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-400 ${scrolled ? "bg-white/90 backdrop-blur-xl shadow-sm" : "bg-transparent"}`}>
      <div className={`${CX} flex items-center justify-between h-16 lg:h-[72px]`}>
        <a href="#" onClick={e => { e.preventDefault(); window.scrollTo({ top: 0, behavior: "smooth" }) }}>
          <img src={img("logo")} alt="Divinity" className={`h-8 lg:h-9 transition-opacity ${scrolled ? "opacity-100" : "opacity-0"}`} />
        </a>
        <div className="hidden lg:flex items-center gap-8">
          {links.map(l => (
            <button key={l.h} onClick={() => scrollTo(l.h)} className={`text-[12px] tracking-[1.5px] font-semibold transition uppercase ${scrolled ? "text-brand-500 hover:text-brand-700" : "text-white/80 hover:text-white"}`}>{l.l}</button>
          ))}
        </div>
      ...
      {mobileOpen && (
        <div className="lg:hidden bg-white border-t border-brand-100 px-6 py-4 shadow-lg">
          {links.map(l => (
  ...
  return (
    <section className="relative h-screen min-h-[600px] max-h-[1100px] flex items-center justify-center overflow-hidden">
      <div className="absolute inset-0">
        <img src={img("heroImg")} alt="" className="w-full h-full object-cover" />
      ...
      <div className="relative z-10 text-center text-white px-6 max-w-3xl mx-auto">
        ...
        <h1 className="font-display text-[40px] sm:text-5xl md:text-6xl lg:text-7xl font-bold mb-6 leading-[1.05]">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          <button onClick={() => scrollTo("guides")} className="px-8 py-3.5 bg-gold-500 text-white rounded-full text-[13px] font-bold tracking-wider hover:bg-gold-600 transition-all hover:shadow-lg uppercase w-full sm:w-auto">
            Meet Your Guide
          </button>
          <button onClick={() => scrollTo("how-it-works")} className="px-8 py-3.5 border border-white/40 text-white rounded-full text-[13px] font-bold tracking-wider hover:bg-white/10 transition uppercase w-full sm:w-auto">
            How It Works
          </button>
        </div>
      </div>
      ...
      <div ref={f.ref} className={`${CX} ${f.cls}`}>
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div className="relative rounded-2xl overflow-hidden shadow-2xl aspect-[4/3]">
            <img src={img("companionsImg")} alt="AI Companions" className="w-full h-full object-cover object-top" />
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              <button onClick={() => scrollTo("guides")} className="px-7 py-3 bg-gold-500 text-white rounded-full text-[13px] font-bold tracking-wider hover:bg-gold-600 transition uppercase">
                Explore Guides
              </button>
    ...
    <section id="explore" className="bg-brand-50 py-20 lg:py-28">
      <div ref={f.ref} className={`${CX} ${f.cls}`}>
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16">
          <div>
            <img src={img("meditate")} alt="Meditation" className="w-full h-56 lg:h-64 object-cover rounded-2xl mb-7" />
          ...
          <div>
            <img src={img("circleGroup")} alt="Circle of faiths" className="w-full h-56 lg:h-64 object-cover object-top rounded-2xl mb-7" />
          ...
          <div className="order-2 lg:order-1">
            ...
            <div className="grid grid-cols-3 gap-2.5 mb-7">
              {[
                ...
                <div key={i} className="flex items-center gap-2 p-2.5 rounded-lg bg-brand-50 border border-brand-100">
                  <img src={img(f.icon)} alt="" className="w-5 h-5 opacity-70" />
                  <span className="text-[13px] text-brand-600 font-medium">{f.name}</span>
                </div>
              ))}
            </div>
            <a href="https://divinityagi.com/circle-of-faiths/" className="text-[14px] font-bold text-gold-600 hover:text-gold-700 transition inline-flex items-center gap-1.5 group">
              Learn about the Circle <svg className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7" /></svg>
            </a>
          </div>
          <div className="order-1 lg:order-2 flex justify-center">
            <img src={img("circleWheel")} alt="Circle of Faiths" className="w-64 md:w-80 lg:w-[400px] animate-[spin_120s_linear_infinite]" />
          </div>
        </div>
      </div>
    </section>
    ...
    <section id="guides" className="bg-brand-50 py-20 lg:py-28">
      <div ref={f.ref} className={`${CX} ${f.cls}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 lg:gap-5 max-w-5xl mx-auto">
          {guides.map((g, i) => (
            <a key={i} href={g.url} target="_blank" rel="noopener noreferrer" className="group relative rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-400 hover:-translate-y-1">
              <div className="relative aspect-[3/4] overflow-hidden">
                <img src={img(g.img)} alt={g.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                <div className="absolute inset-0 bg-gradient-to-t from-brand-900/80 via-brand-900/20 to-transparent" />
                <img src={img(g.icon)} alt="" className="absolute top-3 right-3 w-6 h-6 lg:w-7 lg:h-7 opacity-70" />
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-brand-900/20">
      ...
      <div ref={f.ref} className={`${CX} ${f.cls}`}>
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start">
          <div>
            ...
            <img src={img("taliaCover")} alt="Talia Rosenberg" className="w-full aspect-[16/9] object-cover rounded-2xl mb-6" />
          ...
          <div className="bg-brand-50 rounded-2xl p-6 lg:p-8 border border-brand-100">
            ...
            <div className="mt-7 pt-5 border-t border-brand-200 text-center">
              <a href={guides[1].url} target="_blank" rel="noopener noreferrer" className="px-7 py-3 bg-gold-500 text-white rounded-full text-[13px] font-bold tracking-wider hover:bg-gold-600 transition inline-block uppercase">
                Talk to Talia
              </a>
            </div>
          </div>
...
function HowItWorks() {
  const f = useFade()
  const steps = [
    { n: "01", title: "Choose a Path", desc: "Select a tradition, a philosophy, or simply your curiosity. There's no wrong door.", img: img("quietSpace") },
    { n: "02", title: "Start a Conversation", desc: "Connect with a guide. Ask about meaning, grief, purpose, doubt, or anything on your heart.", img: img("womanPhone") },
    { n: "03", title: "Reflect at Your Own Pace", desc: "Pause, come back tomorrow, switch guides, or go deeper. No timer, no pressure.", img: img("quietGroup") },
  ]
  return (
    <section id="how-it-works" className="bg-brand-50 py-20 lg:py-28">
      <div ref={f.ref} className={`${CX} ${f.cls}`}>
        ...
        <div className="grid md:grid-cols-3 gap-6 lg:gap-8 max-w-5xl mx-auto">
          {steps.map((s, i) => (
            <div key={i} className="rounded-2xl overflow-hidden bg-white border border-brand-100 hover:shadow-xl transition-all group">
...
function Pricing() {
  const f = useFade()
  const plans = [
    { name: "Explorer", price: "Free", sub: "", features: ["5 minutes per day", "Access to 3 guides", "Text conversations", "Basic reflection tools"], cta: "Start Free", href: "#guides", pop: false },
    { name: "Seeker", price: "$9.99", sub: "/mo", features: ["60 minutes per month", "All 9+ guides", "Voice & text", "Conversation history", "Priority access"], cta: "Buy Minutes", href: "https://divinityagi.com/subscribe-now/", pop: true },
    { name: "Devoted", price: "$24.99", sub: "/mo", features: ["Unlimited minutes", "All 9+ guides", "Voice & text", "Full history", "Priority access", "Early access to new guides"], cta: "Buy Minutes", href: "https://divinityagi.com/subscribe-now/", pop: false },
  ]
  return (
    <section id="pricing" className="py-20 lg:py-28">
      <div ref={f.ref} className={`${CX} ${f.cls}`}>
        ...
        <div className="grid md:grid-cols-3 gap-5 lg:gap-6 max-w-4xl mx-auto">
          {plans.map((p, i) => (
            <div key={i} className={`rounded-2xl p-7 lg:p-8 flex flex-col ${p.pop ? "bg-brand-600 text-white shadow-2xl ring-2 ring-gold-400 lg:scale-105" : "bg-brand-50 border border-brand-100"}`}>
              <p className={`text-[11px] tracking-[2px] uppercase font-bold mb-5 ${p.pop ? "text-gold-300" : "text-gold-500"}`}>{p.name}</p>
              <div className="mb-6">
                <span className={`font-display text-[44px] font-bold ${p.pop ? "text-white" : "text-brand-700"}`}>{p.price}</span>
              ...
              <a href={p.href} className={`block text-center px-6 py-3 rounded-full text-[13px] font-bold tracking-wider transition uppercase ${p.pop ? "bg-gold-500 text-white hover:bg-gold-600" : "bg-brand-500 text-white hover:bg-brand-600"}`}>
                {p.cta}
              </a>
            </div>
          ))}
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { t: "Curated Responses", d: "Each guide responds within its tradition", i: "🛡" },
                { t: "Crisis Protocols", d: "Recognizes when professional help is needed", i: "🚨" },
                { t: "Confidential", d: "Private conversations, deletable anytime", i: "🔒" },
                { t: "No Manipulation", d: "No conversion tactics or pressure", i: "✋" },
              ].map((p, i) => (
                <div key={i} className="p-4 rounded-xl bg-white border border-brand-100">
          ...
          <div className="rounded-2xl overflow-hidden shadow-xl">
            <video autoPlay muted loop playsInline poster={img("leadersFrame")} className="w-full rounded-2xl">
    ...
    <section className="py-20 lg:py-28">
      <div ref={f.ref} className={`${CX} ${f.cls}`}>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 max-w-5xl mx-auto">
          {reviews.map((t, i) => (
            <div key={i} className="p-6 rounded-2xl bg-brand-50 border border-brand-100 hover:shadow-lg transition-all">
              ...
              <p className="text-brand-500 text-[14px] leading-relaxed mb-5">"{t.t}"</p>
              <p className="font-bold text-brand-700 text-[14px]">{t.n}</p>
              <p className="text-brand-300 text-[12px]">{t.s}</p>
            </div>
          ))}
        </div>
      </div>
...
function Leaders() {
  const f = useFade()
  return (
    <section id="leaders" className="relative py-24 lg:py-36 overflow-hidden">
      <div className="absolute inset-0">
        <img src={img("verifiedBg")} alt="" className="w-full h-full object-cover" />
      ...
      <div ref={f.ref} className={`relative z-10 max-w-2xl mx-auto px-6 text-center text-white ${f.cls}`}>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          <a href="https://divinityagi.com/spiritual-leaders-and-influencers/" className="px-8 py-3.5 bg-gold-500 text-white rounded-full text-[13px] font-bold tracking-wider hover:bg-gold-600 transition uppercase">Learn More</a>
          ...
          <div>
            ...
            {sent ? (
              ...
              <form onSubmit={e => { e.preventDefault(); if (email.includes("@")) setSent(true) }} className="flex gap-2">
                ...
                <button type="submit" className="px-7 py-3 bg-gold-500 text-white rounded-full text-[13px] font-bold hover:bg-gold-600 transition shrink-0">Subscribe</button>
              </form>
            )}
          </div>
          <div className="text-center">
            <img src={img("phoneStack")} alt="Download" className="w-48 lg:w-56 mx-auto mb-7 drop-shadow-2xl" />
    ...
    <footer className="bg-white border-t border-brand-100 py-12">
      <div className={CX}>
        <div className="flex justify-center mb-6">
          <img src={img("logoApp")} alt="DivinityAGI" className="w-10 h-10" />
        </div>
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2 max-w-md mx-auto mb-10">
          {iconLinks.map((n, i) => (
            <a key={i} href={n.h} className="flex flex-col items-center gap-1.5 p-2 rounded-lg hover:bg-brand-50 transition group">
              <img src={img(n.k)} alt="" className="w-7 h-7 opacity-50 group-hover:opacity-100 transition" />
...
export default function App() {
  ...
  if (path === "/bot" || path === "/bot/") {
    return <Suspense fallback={<div className="min-h-screen bg-[#2d2d2d]" />}><BotPage /></Suspense>
  }
  if (path === "/v2" || path === "/v2/") {
    return <Suspense fallback={<div className="min-h-screen bg-white" />}><V2Page /></Suspense>
  }
  return <LandingPage />
}
